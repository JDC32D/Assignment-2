...
//               position: "9", rings: Filter.sadPlanet )
    ...
    func getFilteredPlanet(position: Int) -> Planet {
        return filteredPlanets[position]
    }
    
    func getFilteredPlanets() -> [Planet]{
        return filteredPlanets
    }
    
    func filterPlanetList(index filter: Int) {
        if filter < 5 {
            filteredPlanets = planets.filter { $0.rings == Filter.allCases[filter] }
        } else {
            filteredPlanets = planets.filter { $0.numMoons == Filter.allCases[filter] }
        }
    }
